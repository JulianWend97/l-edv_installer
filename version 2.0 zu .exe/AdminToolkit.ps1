...
# L-EDV Admin Toolkit v2.0
# (c) 2026 Felix Natterer, Julian Wendland - L-EDV
# UTF-8, CRLF, PowerShell 5+
...
# OPTIMIZED: Kein automatischer Aufruf beim Start mehr - nur noch via LEDV-Installer-Checkbox
...
$Form.Text            = "L-EDV Toolkit  |  IT-Support v2.0"
...
#VERSION VERAENDERN BEIM ANPASSEN DER DATEI
$lblHeaderSub = New-Object System.Windows.Forms.Label
$lblHeaderSub.Text      = "Windows L-EDV Toolkit  |  IT-Support v2.0"
...
$lblCopy.Text      = "(c) 2026 Felix Natterer, Julian Wendland - L-EDV"
$lblCopy.Font      = $fSmall
...
$tabNames = @("LEDV Installer","Wartung","System","Legacy Panels","Software","Debloat","Abschluss","Tools","Portable Apps","Offline Installers")
...
$Script:SideBtns["Offline Installers"].Add_Click({ Switch-Tab "Offline Installers" })
...
# ADDED: Installierte Software anzeigen (Registry-basiert, schnell) + Winget-Integration
Make-Hdr $spSW "Installierte Software" 8 110 900

$Script:lvSoftware = New-Object System.Windows.Forms.ListView
$Script:lvSoftware.Location    = New-Object System.Drawing.Point(8, 138)
$Script:lvSoftware.Size        = New-Object System.Drawing.Size(895, 230)
$Script:lvSoftware.View        = "Details"
$Script:lvSoftware.FullRowSelect = $true
$Script:lvSoftware.GridLines   = $true
$Script:lvSoftware.BackColor   = $cPanel2
$Script:lvSoftware.ForeColor   = $cText
$Script:lvSoftware.Font        = $fSmall
$Script:lvSoftware.Sorting     = "Ascending"
$null = $Script:lvSoftware.Columns.Add("Name",         320)
$null = $Script:lvSoftware.Columns.Add("Version",      120)
$null = $Script:lvSoftware.Columns.Add("Publisher",    220)
$null = $Script:lvSoftware.Columns.Add("Installiert",  130)
$spSW.Controls.Add($Script:lvSoftware)

function Load-InstalledSoftware {
    $Script:lvSoftware.Items.Clear()
    Set-Status "Installierte Software wird geladen..." 20
    [System.Windows.Forms.Application]::DoEvents()
    # OPTIMIZED: Registry direkt lesen statt Get-WmiObject Win32_Product (sehr langsam ~30s)
    $keys = @(
        "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*",
        "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*",
    ...
    $Script:lvSoftware.BeginUpdate()
    foreach ($a in $apps) {
        $item = New-Object System.Windows.Forms.ListViewItem($a.DisplayName)
        $item.SubItems.Add($(if ($a.DisplayVersion) { $a.DisplayVersion } else { "" })) | Out-Null
        $item.SubItems.Add($(if ($a.Publisher)      { $a.Publisher }      else { "" })) | Out-Null
        $dt = $a.InstallDate
        $item.SubItems.Add($(if ($dt -and $dt.Length -ge 8) { "$($dt.Substring(0,4))-$($dt.Substring(4,2))-$($dt.Substring(6,2))" } else { "" })) | Out-Null
        $Script:lvSoftware.Items.Add($item) | Out-Null
    }
    $Script:lvSoftware.EndUpdate()
    Write-Log "Installierte Software geladen: $($apps.Count) Eintraege"
    Set-Status "$($apps.Count) Programme geladen" 100 "ok"
}

function Export-SoftwareList {
    $sfd = New-Object System.Windows.Forms.SaveFileDialog
    $sfd.Filter   = "CSV-Datei (*.csv)|*.csv"
    $sfd.FileName = "$env:COMPUTERNAME-Software-$(Get-Date -Format 'yyyy-MM-dd').csv"
    if ($sfd.ShowDialog() -eq "OK") {
        $lines = @("Name,Version,Publisher,Installiert")
        foreach ($item in $Script:lvSoftware.Items) {
            $lines += "`"$($item.Text)`",`"$($item.SubItems[1].Text)`",`"$($item.SubItems[2].Text)`",`"$($item.SubItems[3].Text)`""
        }
        $lines | Out-File $sfd.FileName -Encoding UTF8
        Write-Log "Software-Liste exportiert: $($sfd.FileName)"
        Set-Status "Software-Liste exportiert: $($sfd.FileName)" 100 "ok"
    }
}

$swBtnY = 376
Make-Btn $spSW "Liste laden / aktualisieren" 8 $swBtnY 200 30 $cPanel2 { Load-InstalledSoftware } | Out-Null
Make-Btn $spSW "Als CSV exportieren"         216 $swBtnY 160 30 $cPanel2 { Export-SoftwareList } | Out-Null

# Suchfeld für Software-Liste
$Script:txSoftSearch = New-Object System.Windows.Forms.TextBox
$Script:txSoftSearch.Location    = New-Object System.Drawing.Point(384, $swBtnY)
$Script:txSoftSearch.Size        = New-Object System.Drawing.Size(200, 28)
$Script:txSoftSearch.BackColor   = $cPanel2
$Script:txSoftSearch.ForeColor   = $cText
$Script:txSoftSearch.BorderStyle = "FixedSingle"
$Script:txSoftSearch.Font        = $fSmall
$Script:txSoftSearch.Text        = "Suchen..."
$Script:txSoftSearch.Add_TextChanged({
    $q = $Script:txSoftSearch.Text.ToLower()
    foreach ($item in $Script:lvSoftware.Items) {
        $item.BackColor = if ($q.Length -gt 2 -and $item.Text.ToLower().Contains($q)) { $cAccent2 } else { $cPanel2 }
    }
})
$spSW.Controls.Add($Script:txSoftSearch)

# ADDED: Winget-Integration
Make-Hdr $spSW "Winget - Windows Package Manager" 8 414 900

Make-Btn $spSW "Winget: Alle Updates installieren" 8 442 240 32 $cPanel2 {
    Write-Log "Winget: Alle Updates"
    Set-Status "Winget Update startet..." 10
    Start-Process "powershell.exe" "-NoProfile -Command `"winget upgrade --all --silent --accept-package-agreements --accept-source-agreements; pause`"" -Verb RunAs
    Set-Status "Winget Update gestartet (externes Fenster)" 50 "ok"
} | Out-Null

Make-Btn $spSW "Winget: Update-Liste anzeigen" 256 442 210 32 $cPanel2 {
    Write-Log "Winget: Veraltete Liste"
    Start-Process "powershell.exe" "-NoProfile -Command `"winget upgrade; pause`""
    Set-Status "Winget Update-Liste geoeffnet" 100 "ok"
} | Out-Null

Make-Btn $spSW "Winget: verfuegbar pruefen" 474 442 190 32 $cPanel2 {
    $wg = Get-Command winget -ErrorAction SilentlyContinue
    if ($wg) {
        Set-Status "Winget verfuegbar: $($wg.Source)" 100 "ok"
        [System.Windows.Forms.MessageBox]::Show("Winget verfuegbar:`n$($wg.Source)","L-EDV","OK","Information") | Out-Null
    } else {
        Set-Status "Winget nicht gefunden - Windows App-Installer benoetigt" 0 "warn"
        [System.Windows.Forms.MessageBox]::Show("Winget nicht gefunden.`nBitte 'App-Installer' aus dem Microsoft Store installieren.","Hinweis","OK","Warning") | Out-Null
    }
    Write-Log "Winget-Check"
} | Out-Null

Make-Btn $spSW "Autostart-Programme anzeigen" 672 442 220 32 $cPanel2 {
    Write-Log "Autostart-Abfrage"
    Set-Status "Autostart wird geladen..." 10
    $startupPaths = @(
    ...
    $out = "=== AUTOSTART ===$nl$nl"
    $out += "--- Startmenü-Ordner ---$nl"
    foreach ($p in $startupPaths) {
        if (Test-Path $p) {
            Get-ChildItem $p -File -ErrorAction SilentlyContinue | ForEach-Object { $out += "  $($_.Name)$nl" }
        }
    }
    $out += "$nl--- Registry ---$nl"
    foreach ($k in $regKeys) {
        if (Test-Path $k) {
            ...
            Get-ItemProperty $k -ErrorAction SilentlyContinue | Get-Member -MemberType NoteProperty |
                ...
                ForEach-Object { $out += "  $($_.Name): $((Get-ItemProperty $k)."$($_.Name)")$nl" }
        }
    }
    $fAS = New-Object System.Windows.Forms.Form
    $fAS.Text = "Autostart-Programme - $env:COMPUTERNAME"
    $fAS.Size = New-Object System.Drawing.Size(780, 520)
    $fAS.BackColor = $cBG; $fAS.StartPosition = "CenterScreen"
    $tbAS = New-Object System.Windows.Forms.TextBox
    $tbAS.Multiline = $true; $tbAS.ScrollBars = "Both"; $tbAS.ReadOnly = $true
    $tbAS.Dock = "Fill"; $tbAS.BackColor = $cPanel; $tbAS.ForeColor = $cText; $tbAS.Font = $fMono; $tbAS.Text = $out
    $fAS.Controls.Add($tbAS); $fAS.ShowDialog() | Out-Null
    Set-Status "Autostart-Übersicht angezeigt" 100 "ok"
    Write-Log "Autostart-Übersicht angezeigt"
} | Out-Null
...
# ============================================================
$pgOI = $Script:Pages["Offline Installers"]
Make-Hdr $pgOI "Offline Installers" 8 8 900
...
Make-Btn $spW "Systeminfo-Fenster" 686 158 196 32 $cPanel2 {
    ...
    if ($ci) {
        # CHANGED: Win11-Bugfix - Build >= 22000 = Windows 11
        $osReg  = Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion" -ErrorAction SilentlyContinue
        $build  = if ($osReg.CurrentBuildNumber) { [int]$osReg.CurrentBuildNumber } else { [int]$ci.OsBuildNumber }
        $dispVer= $osReg.DisplayVersion
        $winName= if ($build -ge 22000) { "Windows 11" } else { "Windows 10" }
        $winStr = "$winName $dispVer (Build $build)"
        $out += "Windows  : $winStr$nl"
...
Make-Btn $spW "Toolkit-Ordner nach Schliessen loeschen" 8 278 280 32 $cAccent2 {
    $r = [System.Windows.Forms.MessageBox]::Show(
        "ACHTUNG: Toolkit-Dateien werden nach GUI-Schliessen geloescht!" + [Environment]::NewLine + $Script:RootDir + [Environment]::NewLine + "Der Ordner L-EDV-Dateien bleibt erhalten." + [Environment]::NewLine + "Fortfahren?",
    ...
    if ($r -eq "Yes") {
        ...
        $content = @(
            ...
            "set `"ROOT=$root`"",
            "for /d %%D in (`"%root%\*`") do if /I not `"%%~nxD`"==`"L-EDV-Dateien`" rd /s /q `"%%~fD`"",
            "for %%F in (`"%root%\*`") do del /f /q `"%%~fF`"",
            "del `"%~f0`""
        ...
        Set-Status "Toolkit wird nach Schliessen geloescht - L-EDV-Dateien bleibt" 100 "warn"
        [System.Windows.Forms.MessageBox]::Show("OK. Toolkit-Dateien werden nach dem Schliessen geloescht.`nL-EDV-Dateien bleibt erhalten.","L-EDV","OK","Information") | Out-Null
    }
} | Out-Null
...
Make-Btn $pgInfo "Systeminfo laden" 10 430 160 28 $cPanel2 {
    $ci  = Get-ComputerInfo -ErrorAction SilentlyContinue
    $nl  = [Environment]::NewLine
    # CHANGED: Win11-Erkennung via Registry statt Get-ComputerInfo.WindowsProductName
    $osReg3 = Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion" -ErrorAction SilentlyContinue
    $build3 = if ($osReg3.CurrentBuildNumber) { [int]$osReg3.CurrentBuildNumber } else { [int]$ci.OsBuildNumber }
    $dv3    = $osReg3.DisplayVersion
    $wn3    = if ($build3 -ge 22000) { "Windows 11" } else { "Windows 10" }
    $winStr3= "$wn3 $dv3 (Build $build3)"
    $Script:dbInfo.Text = "Hostname: $env:COMPUTERNAME${nl}Windows : $winStr3${nl}RAM     : $([math]::Round($ci.CsTotalPhysicalMemory/1GB,1)) GB${nl}CPU     : $($ci.CsProcessors[0].Name)$nl"
    Set-Status "Systeminfo geladen" 100 "ok"
} | Out-Null
...
$y=62
foreach($d in $twDefs){ $fg=if($d[0] -eq "OneDrive"){$cWarn}else{$cText}; $Script:ledvChecks[$d[0]] = New-ADTCheck $spI $d[1] 474 $y 432 $false $fg; $y += 22 }

function Set-LedvInstallProfile {
    param([string]$Profile)
    foreach ($cb in $Script:ledvChecks.Values) { $cb.Checked = $false }
    $selection = switch ($Profile) {
        "Basis" { @("Firefox","Thunderbird","7zip","Adobe","7zip","PDF24","VLC","TVQS","SysRestore") }
        "Homeoffice" { @("Firefox","Chrome","Thunderbird","Libre","Adobe","7zip","PDF24","Irfan","Teams","TVQS","SysRestore") }
        "Schule" { @("Firefox","Chrome","Libre","PDF24","7zip","Irfan","VLC","TVQS","SysRestore") }
...
# ---- Preset-Datei: Voreinstellungen speichern / laden ----
$Script:PresetFile = Join-Path $Script:RootDir "ledv_presets.json"

function Save-LedvPreset {
    param([string]$PresetName)
    if ([string]::IsNullOrWhiteSpace($PresetName)) {
        [System.Windows.Forms.MessageBox]::Show("Bitte einen Preset-Namen eingeben.","Hinweis","OK","Warning") | Out-Null; return
    }
    # Alle aktuellen Checkbox-Zustände sammeln
    $state = @{}
    foreach ($kv in $Script:ledvChecks.GetEnumerator()) { $state[$kv.Key] = $kv.Value.Checked }
    # Vorhandene Presets laden oder neues Objekt
    $all = @{}
    if (Test-Path $Script:PresetFile) {
        try { $all = Get-Content $Script:PresetFile -Raw -Encoding UTF8 | ConvertFrom-Json -ErrorAction Stop
              $tmp = @{}; $all.PSObject.Properties | ForEach-Object { $tmp[$_.Name] = $_.Value }; $all = $tmp
        } catch { $all = @{} }
    }
    $all[$PresetName] = $state
    $all | ConvertTo-Json -Depth 4 | Set-Content $Script:PresetFile -Encoding UTF8
    Write-Log "LEDV Preset gespeichert: $PresetName"
    Set-Status "Preset '$PresetName' gespeichert" 100 "ok"
    Update-PresetCombo
}

function Load-LedvPreset {
    param([string]$PresetName)
    if (-not (Test-Path $Script:PresetFile)) { Set-Status "Keine Preset-Datei gefunden" 0 "warn"; return }
    try {
        $all = Get-Content $Script:PresetFile -Raw -Encoding UTF8 | ConvertFrom-Json -ErrorAction Stop
        $preset = $all.$PresetName
        if (-not $preset) { Set-Status "Preset '$PresetName' nicht gefunden" 0 "warn"; return }
        foreach ($kv in $Script:ledvChecks.GetEnumerator()) {
            $val = $preset.($kv.Key)
            if ($null -ne $val) { $kv.Value.Checked = [bool]$val }
        }
        Write-Log "LEDV Preset geladen: $PresetName"
        Set-Status "Preset '$PresetName' geladen" 100 "ok"
    } catch { Write-Log "Preset laden Fehler: $_" "ERROR"; Set-Status "Preset Fehler: $_" 0 "err" }
}

function Delete-LedvPreset {
    param([string]$PresetName)
    if (-not (Test-Path $Script:PresetFile)) { return }
    try {
        $all = Get-Content $Script:PresetFile -Raw -Encoding UTF8 | ConvertFrom-Json -ErrorAction Stop
        $tmp = @{}; $all.PSObject.Properties | ForEach-Object { if ($_.Name -ne $PresetName) { $tmp[$_.Name] = $_.Value } }
        $tmp | ConvertTo-Json -Depth 4 | Set-Content $Script:PresetFile -Encoding UTF8
        Write-Log "LEDV Preset geloescht: $PresetName"
        Set-Status "Preset '$PresetName' geloescht" 100 "ok"
        Update-PresetCombo
    } catch {}
}

function Update-PresetCombo {
    $Script:cmbPreset.Items.Clear()
    if (Test-Path $Script:PresetFile) {
        try {
            $all = Get-Content $Script:PresetFile -Raw -Encoding UTF8 | ConvertFrom-Json -ErrorAction Stop
            $all.PSObject.Properties | ForEach-Object { $Script:cmbPreset.Items.Add($_.Name) | Out-Null }
        } catch {}
    }
}

# Preset-UI - Row oberhalb Profile-Header
$pPreset = New-Object System.Windows.Forms.Panel
$pPreset.Location  = New-Object System.Drawing.Point(8, 350)
$pPreset.Size      = New-Object System.Drawing.Size(888, 32)
$pPreset.BackColor = $cPanel2
$spI.Controls.Add($pPreset)

$lblPreset = New-Object System.Windows.Forms.Label
$lblPreset.Text      = "Eigenes Preset:"
$lblPreset.Font      = $fSmall
$lblPreset.ForeColor = $cSub
$lblPreset.Location  = New-Object System.Drawing.Point(6, 8)
$lblPreset.Size      = New-Object System.Drawing.Size(100, 16)
$pPreset.Controls.Add($lblPreset)

$Script:cmbPreset = New-Object System.Windows.Forms.ComboBox
$Script:cmbPreset.Location    = New-Object System.Drawing.Point(110, 4)
$Script:cmbPreset.Size        = New-Object System.Drawing.Size(200, 22)
$Script:cmbPreset.BackColor   = $cPanel
$Script:cmbPreset.ForeColor   = $cText
$Script:cmbPreset.FlatStyle   = "Flat"
$Script:cmbPreset.Font        = $fSmall
$pPreset.Controls.Add($Script:cmbPreset)

$btnLoadPreset = New-Object System.Windows.Forms.Button
$btnLoadPreset.Text      = "Laden"
$btnLoadPreset.Location  = New-Object System.Drawing.Point(318, 3)
$btnLoadPreset.Size      = New-Object System.Drawing.Size(70, 24)
$btnLoadPreset.Font      = $fSmall
$btnLoadPreset.BackColor = $cPanel
$btnLoadPreset.ForeColor = $cText
$btnLoadPreset.FlatStyle = "Flat"
$btnLoadPreset.FlatAppearance.BorderColor = $cBorder
$btnLoadPreset.Cursor    = [System.Windows.Forms.Cursors]::Hand
$btnLoadPreset.Add_Click({ Load-LedvPreset $Script:cmbPreset.Text })
$pPreset.Controls.Add($btnLoadPreset)

$Script:txPresetName = New-Object System.Windows.Forms.TextBox
$Script:txPresetName.Location    = New-Object System.Drawing.Point(398, 4)
$Script:txPresetName.Size        = New-Object System.Drawing.Size(160, 22)
$Script:txPresetName.BackColor   = $cPanel
$Script:txPresetName.ForeColor   = $cText
$Script:txPresetName.BorderStyle = "FixedSingle"
$Script:txPresetName.Font        = $fSmall
$Script:txPresetName.Text        = "Mein Preset"
$pPreset.Controls.Add($Script:txPresetName)

$btnSavePreset = New-Object System.Windows.Forms.Button
$btnSavePreset.Text      = "Speichern"
$btnSavePreset.Location  = New-Object System.Drawing.Point(566, 3)
$btnSavePreset.Size      = New-Object System.Drawing.Size(80, 24)
$btnSavePreset.Font      = $fSmall
$btnSavePreset.BackColor = $cPanel
$btnSavePreset.ForeColor = $cText
$btnSavePreset.FlatStyle = "Flat"
$btnSavePreset.FlatAppearance.BorderColor = $cBorder
$btnSavePreset.Cursor    = [System.Windows.Forms.Cursors]::Hand
$btnSavePreset.Add_Click({ Save-LedvPreset $Script:txPresetName.Text })
$pPreset.Controls.Add($btnSavePreset)

$btnDelPreset = New-Object System.Windows.Forms.Button
$btnDelPreset.Text      = "Loeschen"
$btnDelPreset.Location  = New-Object System.Drawing.Point(654, 3)
$btnDelPreset.Size      = New-Object System.Drawing.Size(80, 24)
$btnDelPreset.Font      = $fSmall
$btnDelPreset.BackColor = $cPanel
$btnDelPreset.ForeColor = $cText
$btnDelPreset.FlatStyle = "Flat"
$btnDelPreset.FlatAppearance.BorderColor = $cBorder
$btnDelPreset.Cursor    = [System.Windows.Forms.Cursors]::Hand
$btnDelPreset.Add_Click({ Delete-LedvPreset $Script:cmbPreset.Text; $Script:cmbPreset.Text = "" })
$pPreset.Controls.Add($btnDelPreset)

Update-PresetCombo

Make-Hdr $spI "Profile" 8 390 888
Make-Btn $spI "Basis Privatkunde" 16 420 160 28 $cPanel2 { Set-LedvInstallProfile "Basis" } | Out-Null
Make-Btn $spI "Homeoffice" 184 420 140 28 $cPanel2 { Set-LedvInstallProfile "Homeoffice" } | Out-Null
Make-Btn $spI "Schule / Studium" 332 420 150 28 $cPanel2 { Set-LedvInstallProfile "Schule" } | Out-Null
Make-Btn $spI "Nur Wartung" 490 420 130 28 $cPanel2 { Set-LedvInstallProfile "Wartung" } | Out-Null
Make-Btn $spI "Auswahl leeren" 628 420 130 28 $cPanel2 { foreach ($cb in $Script:ledvChecks.Values) { $cb.Checked = $false }; Set-Status "Auswahl geleert" 100 "ok" } | Out-Null

Make-Hdr $spI "Ausfuehren" 8 462 888
Make-Lbl $spI "Kundenname:" 16 500 100 16 $fSmall $cSub | Out-Null
$Script:txLedvCustomer = New-Object System.Windows.Forms.TextBox
$Script:txLedvCustomer.Location = New-Object System.Drawing.Point(116,496)
...
$Script:btnLedvStart = Make-Btn $spI "Ausgewaehlte Installation starten" 370 492 260 36 $cAccent { Invoke-AdminLedvInstall }
...
# Timer-basiertes Status-Update aus dem Runspace in den GUI-Thread
$Script:LedvRunspace    = $null
$Script:LedvAsyncResult = $null
$Script:LedvStatusQueue = [System.Collections.Concurrent.ConcurrentQueue[string]]::new()
$Script:LedvRunning     = $false

$Script:LedvTimer = New-Object System.Windows.Forms.Timer
$Script:LedvTimer.Interval = 300
$Script:LedvTimer.Add_Tick({
    # Statusmeldungen aus dem Hintergrund-Thread in die GUI pumpen
    $msg = $null
    while ($Script:LedvStatusQueue.TryDequeue([ref]$msg)) {
        if ($msg -match "^PROGRESS:(\d+):(.+)$") {
            Set-Status $Matches[2] ([int]$Matches[1])
        } elseif ($msg -eq "DONE") {
            $Script:LedvTimer.Stop()
            $Script:LedvRunning = $false
            $Script:btnLedvStart.Enabled = $true
            Set-Status "LEDV Installation abgeschlossen - Neustart empfohlen" 100 "ok"
            [System.Windows.Forms.MessageBox]::Show("Installation abgeschlossen.`nBitte Notebook neu starten.","L-EDV Installer","OK","Information") | Out-Null
        } elseif ($msg -match "^ERROR:(.+)$") {
            $Script:LedvTimer.Stop()
            $Script:LedvRunning = $false
            $Script:btnLedvStart.Enabled = $true
            Set-Status "Fehler: $($Matches[1])" 0 "err"
            [System.Windows.Forms.MessageBox]::Show("Fehler aufgetreten:`n" + $Matches[1],"L-EDV Installer","OK","Error") | Out-Null
        }
    }
})

function Invoke-AdminLedvInstall {
    if ($Script:LedvRunning) {
        [System.Windows.Forms.MessageBox]::Show("Installation laeuft bereits - bitte warten.","Hinweis","OK","Warning") | Out-Null
        return
    }
    $name = $Script:txLedvCustomer.Text.Trim()
    if (-not $name) {
        [System.Windows.Forms.MessageBox]::Show("Bitte Kundennamen eingeben.","Hinweis","OK","Warning") | Out-Null
        return
    }

    # Snapshot der Checkbox-Zustände (GUI-Thread) VOR dem Runspace
    $snapChecks   = @{}
    foreach ($kv in $Script:ledvChecks.GetEnumerator()) { $snapChecks[$kv.Key] = $kv.Value.Checked }
    $snapPackages = @{}
    foreach ($kv in $Script:ledvPackages.GetEnumerator()) { $snapPackages[$kv.Key] = $kv.Value }

    $Script:LedvRunning = $true
    $Script:btnLedvStart.Enabled = $false
    $Script:LedvStatusQueue = [System.Collections.Concurrent.ConcurrentQueue[string]]::new()
    Set-Status "LEDV Installation gestartet (Hintergrund)..." 2

    # Alle benötigten Werte für den Runspace als einfache Variablen
    $rsLogFile      = $Script:LogFile
    $rsRunLogFile   = $Script:RunLogFile
    $rsLogDir       = $Script:LogDir
    $rsLedvFilesDir = $Script:LedvFilesDir
    $rsQSExe        = $Script:QuickSupportExe
    $rsQueue        = $Script:LedvStatusQueue
    $rsName         = $name

    $Script:LedvRunspace = [runspacefactory]::CreateRunspace()
    $Script:LedvRunspace.ApartmentState = "STA"
    $Script:LedvRunspace.ThreadOptions  = "ReuseThread"
    $Script:LedvRunspace.Open()

    $ps = [powershell]::Create()
    $ps.Runspace = $Script:LedvRunspace

    $ps.AddScript({
        param($Queue,$LogFile,$RunLogFile,$LogDir,$LedvFilesDir,$QSExe,$Name,$Checks,$Packages)
        ...
        function RS-Log {
            ...
            try { Add-Content -Path $RunLogFile -Value $line -Encoding UTF8 -ErrorAction SilentlyContinue } catch {}
        }
        function RS-Status {
            param([string]$Msg,[int]$Pct=0)
            $Queue.Enqueue("PROGRESS:$Pct`:$Msg")
        }
        function RS-ChocoExe {
            $p = if ($env:ChocolateyInstall) { Join-Path $env:ChocolateyInstall "bin\choco.exe" } else { $null }
            if ($p -and (Test-Path $p)) { return $p }
            $d = "C:\ProgramData\chocolatey\bin\choco.exe"
            if (Test-Path $d) { return $d }
            return $null
        }

        try {
            RS-Log "LEDV Runspace: Start fuer $Name"

            # Protokoll
            if ($Checks["Protocols"]) {
                $ts = Get-Date -Format "yyyyMMdd_HHmm"
                $lf = Join-Path $LogDir ("Protokoll-$Name-$ts.log")
                Get-ComputerInfo | Out-File $lf -Encoding UTF8 -ErrorAction SilentlyContinue
                RS-Log "LEDV: Protokoll $lf"
            }

            # Chocolatey einrichten
            $selected = @($Packages.Keys | Where-Object { $Checks[$_] })
            if ($selected.Count -gt 0 -and -not (RS-ChocoExe)) {
                RS-Status "Chocolatey wird eingerichtet..." 8
                Set-ExecutionPolicy Bypass -Scope Process -Force
                [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12
                $src = (Invoke-WebRequest -Uri 'https://community.chocolatey.org/install.ps1' -UseBasicParsing).Content
                Invoke-Expression $src
                $env:ChocolateyInstall = "C:\ProgramData\chocolatey"
                $env:PATH = "$env:ChocolateyInstall\bin;" + $env:PATH
                RS-Log "LEDV: Chocolatey eingerichtet"
            }

            # Software installieren
            $total = [Math]::Max($selected.Count, 1); $done = 0
            foreach ($key in $selected) {
                $disp = $Packages[$key][0]; $pkg = $Packages[$key][1]
                RS-Status "Installiere $disp..." ([int](($done / $total) * 50) + 15)
                $choco = RS-ChocoExe
                if ($choco) {
                    & $choco install $pkg -fy --no-progress 2>&1 | ForEach-Object { RS-Log "choco: $_" }
                }
                RS-Log "LEDV: $disp installiert"
                $done++
            }

            # TeamViewer QuickSupport
            if ($Checks["TVQS"]) {
                RS-Status "TeamViewer QuickSupport wird geladen..." 70
                try {
                    if (-not (Test-Path $LedvFilesDir)) { New-Item -ItemType Directory -Path $LedvFilesDir -Force | Out-Null }
                    $url = "https://customdesignservice.teamviewer.com/download/windows/v15/6mes7x8/TeamViewerQS_x64.exe"
                    [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12
                    Invoke-WebRequest -Uri $url -OutFile $QSExe -UseBasicParsing -ErrorAction Stop
                    $sh = New-Object -ComObject WScript.Shell
                    $lnk = $sh.CreateShortcut("C:\Users\Public\Desktop\Fernwartung-L-EDV.lnk")
                    $lnk.TargetPath = $QSExe; $lnk.WorkingDirectory = $LedvFilesDir; $lnk.Save()
                    RS-Log "LEDV: TV QS bereitgestellt"
                } catch { RS-Log "LEDV: TV QS Fehler $_" "ERROR" }
            }

            # Tweaks
            if ($Checks["Bloatware"]) {
                RS-Status "Bloatware wird entfernt..." 75
                foreach ($p in @("*Microsoft.BingNews*","*Microsoft.GetHelp*","*Microsoft.People*","*Microsoft.SkypeApp*","*Microsoft.YourPhone*","*Microsoft.ZuneMusic*","*Microsoft.ZuneVideo*","*Microsoft.Clipchamp*","*MicrosoftSolitaireCollection*","*Microsoft.PowerAutomateDesktop*","*Duolingo*","*CandyCrush*","*Spotify*")) {
                    Get-AppxPackage -AllUsers $p -ErrorAction SilentlyContinue | Remove-AppxPackage -AllUsers -ErrorAction SilentlyContinue
                }
                RS-Log "LEDV: Bloatware entfernt"
            }
            if ($Checks["EdgeSC"]) { Set-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer" DisableEdgeDesktopShortcutCreation -Type DWord -Value 1 -ErrorAction SilentlyContinue; RS-Log "LEDV: EdgeSC" }
            if ($Checks["AppSuggest"]) {
                $p = "HKCU:\Software\Microsoft\Windows\CurrentVersion\ContentDeliveryManager"
                foreach ($k in @("ContentDeliveryAllowed","OemPreInstalledAppsEnabled","PreInstalledAppsEnabled","SilentInstalledAppsEnabled","SystemPaneSuggestionsEnabled")) { Set-ItemProperty $p $k -Type DWord -Value 0 -ErrorAction SilentlyContinue }
                RS-Log "LEDV: AppSuggest deaktiviert"
            }
            if ($Checks["Wifi"]) {
                $p = "HKLM:\SOFTWARE\Microsoft\WcmSvc\wifinetworkmanager\config"
                if (-not (Test-Path $p)) { New-Item $p -Force | Out-Null }
                Set-ItemProperty $p AutoConnectAllowedOEM -Type DWord -Value 0 -ErrorAction SilentlyContinue
                Set-ItemProperty $p WiFISenseAllowed -Type DWord -Value 0 -ErrorAction SilentlyContinue
                RS-Log "LEDV: WiFi Sense deaktiviert"
            }
            if ($Checks["F8"]) { bcdedit /set "{current}" BootMenuPolicy Legacy 2>&1 | Out-Null; RS-Log "LEDV: F8 aktiviert" }
            if ($Checks["OneDrive"]) {
                taskkill.exe /F /IM OneDrive.exe 2>&1 | Out-Null
                $s = "$env:SystemRoot\System32\OneDriveSetup.exe"
                if (Test-Path $s) { & $s /uninstall 2>&1 | Out-Null }
                RS-Log "LEDV: OneDrive entfernt"
            }
            if ($Checks["Annoyances"]) {
                Set-ItemProperty "HKLM:\SYSTEM\CurrentControlSet\Control\FileSystem" LongPathsEnabled -Type DWord -Value 1 -ErrorAction SilentlyContinue
                Set-ItemProperty "HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Power" HiberbootEnabled -Type DWord -Value 0 -ErrorAction SilentlyContinue
                Set-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Search" SearchboxTaskbarMode -Type DWord -Value 0 -ErrorAction SilentlyContinue
                Set-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" LaunchTo -Type DWord -Value 1 -ErrorAction SilentlyContinue
                RS-Log "LEDV: Annoyances entfernt"
            }
            if ($Checks["SysRestore"]) {
                Enable-ComputerRestore -Drive "C:\" -ErrorAction SilentlyContinue
                vssadmin resize shadowstorage /on=c: /for=c: /maxsize=5% 2>&1 | Out-Null
                RS-Log "LEDV: Systemwiederherstellung aktiviert"
            }
            if ($Checks["DesktopIcons"]) {
                $regPath = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel"
                if (-not (Test-Path $regPath)) { New-Item $regPath -Force | Out-Null }
                @("{20D04FE0-3AEA-1069-A2D8-08002B30309D}","{59031a47-3f72-44a7-89c5-5595fe6b30ee}","{645FF040-5081-101B-9F08-00AA002F954E}","{F02C1A0D-BE21-4350-88B0-7367FC96EF3C}") |
                    ForEach-Object { Set-ItemProperty $regPath $_ -Type DWord -Value 0 -ErrorAction SilentlyContinue }
                RS-Log "LEDV: Desktop-Icons eingeblendet"
            }

            # Windows Updates - mit detailliertem Status
            if ($Checks["Updates"]) {
                RS-Status "Windows Updates: NuGet wird installiert..." 85
                Install-PackageProvider -Name NuGet -Force -ErrorAction SilentlyContinue | Out-Null
                RS-Status "Windows Updates: PSWindowsUpdate Modul wird geladen..." 87
                Install-Module PSWindowsUpdate -Force -ErrorAction SilentlyContinue | Out-Null
                RS-Status "Windows Updates: Verfuegbare Updates werden gesucht..." 89
                $updates = Get-WindowsUpdate -ErrorAction SilentlyContinue
                if ($updates -and $updates.Count -gt 0) {
                    $ui = 0
                    foreach ($upd in $updates) {
                        $ui++
                        RS-Status ("Windows Update $ui/" + $updates.Count + ": " + $upd.Title.Substring(0,[Math]::Min(60,$upd.Title.Length)) + "...") ([int](89 + ($ui / $updates.Count) * 10))
                        RS-Log "WU: Installiere $($upd.Title)"
                        Install-WindowsUpdate -KBArticleID $upd.KB -AcceptAll -IgnoreReboot -ErrorAction SilentlyContinue | Out-Null
                    }
                    RS-Log "LEDV: Windows Updates installiert ($($updates.Count) Updates)"
                } else {
                    RS-Status "Windows Updates: Keine Updates verfuegbar / bereits aktuell" 99
                    RS-Log "LEDV: Keine Windows Updates gefunden"
                }
            }

            RS-Log "LEDV Runspace: abgeschlossen fuer $Name"
            $Queue.Enqueue("DONE")

        } catch {
            RS-Log "LEDV Runspace FATAL: $_" "ERROR"
            $Queue.Enqueue("ERROR:$_")
        }

    }).AddParameters(@{
        Queue        = $rsQueue
        LogFile      = $rsLogFile
        RunLogFile   = $rsRunLogFile
        LogDir       = $rsLogDir
        LedvFilesDir = $rsLedvFilesDir
        QSExe        = $rsQSExe
        Name         = $rsName
        Checks       = $snapChecks
        Packages     = $snapPackages
    }) | Out-Null

    $Script:LedvAsyncResult = $ps.BeginInvoke()
    $Script:LedvTimer.Start()
    Write-Log "LEDV Runspace gestartet fuer $name"
}
...
function Get-CompletionReportText {
    ...
    if ($ci) {
        # CHANGED: Win11-Bugfix via Registry Build-Nummer
        $osReg2  = Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion" -ErrorAction SilentlyContinue
        $build2  = if ($osReg2.CurrentBuildNumber) { [int]$osReg2.CurrentBuildNumber } else { [int]$ci.OsBuildNumber }
        $dv2     = $osReg2.DisplayVersion
        $wn2     = if ($build2 -ge 22000) { "Windows 11" } else { "Windows 10" }
        $out += "Windows     : $wn2 $dv2 (Build $build2)" + $nl
    }
...
function ConvertTo-LedvHtmlText {
    param([string]$Text)
    if ($null -eq $Text) { return "" }
    return [System.Security.SecurityElement]::Escape($Text)
}

function Get-LedvInstalledSoftwareSummary {
    $lines = New-Object System.Collections.ArrayList
    if (Test-Path $Script:RunLogFile) {
        Get-Content -Path $Script:RunLogFile -ErrorAction SilentlyContinue | ForEach-Object {
            if ($_ -match "Choco install: (.+)$") { [void]$lines.Add($Matches[1]) }
            elseif ($_ -match "Installiere (.+?)\.\.\.") { [void]$lines.Add($Matches[1]) }
            elseif ($_ -match "installiert: (.+)$") { [void]$lines.Add($Matches[1]) }
            elseif ($_ -match "TeamViewer QuickSupport") { [void]$lines.Add("TeamViewer QuickSupport") }
        }
    }
    $clean = $lines | Where-Object { $_ } | Sort-Object -Unique
    if ($clean) { return @($clean) }
    return @("Keine durch das Toolkit protokollierten Softwareinstallationen gefunden.")
}

function Get-LedvActionSummary {
    $items = New-Object System.Collections.ArrayList
    function Add-HandoutAction {
        param([string]$Text)
        if ([string]::IsNullOrWhiteSpace($Text)) { return }
        if (-not ($items -contains $Text)) { [void]$items.Add($Text) }
    }
    if (Test-Path $Script:RunLogFile) {
        Get-Content -Path $Script:RunLogFile -ErrorAction SilentlyContinue | ForEach-Object {
            if ($_ -match "\] \[(INFO|WARN|ERROR)\] (.+)$") {
                $msg = $Matches[2]
                if ($msg -match "^(Tab:|Debloat Unterbereich:|L-EDV Admin Toolkit gestartet|GUI bereit|L-EDV Admin Toolkit beendet|Log geoeffnet|Toolkit-Ordner geoeffnet|Portable Apps:|Offline Installer geladen:|Handout Formular geoeffnet|Handout gespeichert:|Abschluss-Check ausgefuehrt|LEDV Runspace gestartet|LEDV Runspace: Start|LEDV Runspace: abgeschlossen|Integriertes .* geoeffnet|LEDV Profil gewaehlt:|LEDV Preset |Preset )") { return }

                switch -Regex ($msg) {
                    "^Desktop-Symbole gesetzt:" {
                        Add-HandoutAction "Desktop-Symbole fuer Dieser PC und Benutzerordner wurden eingerichtet."
                        break
                    }
                    "^LEDV: (.+) installiert$" {
                        Add-HandoutAction ("Software installiert: " + $Matches[1])
                        break
                    }
                    "^(.+) installiert$" {
                        Add-HandoutAction ("Software installiert: " + $Matches[1])
                    ...
                    "^LEDV: TV QS bereitgestellt|^LEDV: TeamViewer QuickSupport bereitgestellt|^TV QS installiert|^TV QuickSupport bereits vorhanden" {
                        Add-HandoutAction "TeamViewer QuickSupport wurde fuer Fernwartung bereitgestellt."
                        break
                    }
                    "^TV Host entfernt|^TeamViewer Sollzustand:" {
                        Add-HandoutAction "TeamViewer Host wurde entfernt; QuickSupport ist als gewuenschte Fernwartungsloesung vorgesehen."
                        break
                    }
                    "^Debloat App entfernt: (.+)$" {
                        Add-HandoutAction ("Windows-App entfernt: " + $Matches[1])
                        break
                    }
                    "^Debloat integriert: Bloatware abgeschlossen$|^LEDV: Bloatware entfernt$" {
                        Add-HandoutAction "Ausgewaehlte vorinstallierte Windows-Apps/Bloatware wurden entfernt."
                        break
                    }
                    "^Debloat integriert: Datenschutz abgeschlossen$" {
                        Add-HandoutAction "Ausgewaehlte Datenschutz- und Telemetrie-Einstellungen wurden angepasst."
                        break
                    }
                    "^Debloat integriert: Xbox Gaming abgeschlossen$" {
                        Add-HandoutAction "Ausgewaehlte Xbox-/Gaming-Komponenten wurden entfernt oder deaktiviert."
                        break
                    }
                    "^Debloat integriert: Rechtsklick abgeschlossen$" {
                        Add-HandoutAction "Ausgewaehlte Explorer-, Taskleisten- und Rechtsklick-Einstellungen wurden angepasst."
                        break
                    }
                    "^LEDV: AppSuggest deaktiviert$" {
                        Add-HandoutAction "Windows App-Vorschlaege und Content-Empfehlungen wurden deaktiviert."
                        break
                    }
                    "^LEDV: WiFi Sense deaktiviert$" {
                        Add-HandoutAction "WiFi Sense / automatische WLAN-Vorschlaege wurden deaktiviert."
                        break
                    }
                    "^LEDV: EdgeSC$" {
                        Add-HandoutAction "Automatische Microsoft-Edge-Desktopverknuepfungen wurden deaktiviert."
                        break
                    }
                    "^LEDV: F8 aktiviert$" {
                        Add-HandoutAction "Erweitertes F8-Startmenue wurde aktiviert."
                        break
                    }
                    "^LEDV: OneDrive entfernt$" {
                        Add-HandoutAction "OneDrive wurde entfernt."
                        break
                    }
                    "^LEDV: Annoyances entfernt$" {
                        Add-HandoutAction "Ausgewaehlte Windows-Komfort- und Explorer-Einstellungen wurden angepasst."
                        break
                    }
                    "^LEDV: Desktop-Icons eingeblendet$" {
                        Add-HandoutAction "Desktop-Symbole wurden eingeblendet."
                        break
                    }
                    "^LEDV: Systemwiederherstellung aktiviert$|^Systemwiederherstellung aktiviert$" {
                        Add-HandoutAction "Systemwiederherstellung wurde aktiviert."
                        break
                    }
                    "^Wiederherstellungspunkt erstellt$" {
                        Add-HandoutAction "Ein Windows-Wiederherstellungspunkt wurde erstellt."
                        break
                    }
                    "^RegBackup aktiviert$" {
                        Add-HandoutAction "Registry-Backup wurde aktiviert."
                        break
                    }
                    "^LEDV: Windows Updates installiert \((.+)\)$" {
                        Add-HandoutAction ("Windows Updates wurden installiert (" + $Matches[1] + ").")
                        break
                    }
                    "^LEDV: Keine Windows Updates gefunden$" {
                        Add-HandoutAction "Windows Update wurde geprueft; es wurden keine offenen Updates gefunden."
                        break
                    }
                    "^Temp-Bereinigung$|^Temp geloescht:" {
                        Add-HandoutAction "Temporaere Dateien wurden bereinigt."
                        break
                    }
                    "^DNS Cache geleert$" {
                        Add-HandoutAction "DNS-Cache wurde geleert."
                        break
                    }
                    "^Netzwerk-Reset$" {
                        Add-HandoutAction "Netzwerk-Reset wurde gestartet."
                        break
                    }
                    "^SFC Scan$|^DISM$|^SFC\+DISM$" {
                        Add-HandoutAction ("Systemreparatur ausgefuehrt: " + $msg)
                        break
                    }
                    "^Offline Installer beendet: (.+)$" {
                        Add-HandoutAction ("Offline-Installer ausgefuehrt: " + (Split-Path $Matches[1] -Leaf))
                        break
                    }
                    "^Portable gestartet:" {
                        break
    ...
    $clean = $items | Where-Object { $_ } | Select-Object -Last 80
    if ($clean) { return @($clean) }
    return @()
}

function Get-LedvSystemHandoutData {
    # OPTIMIZED: Get-ComputerInfo (~3-8s) ersetzt durch gezielte CIM-Abfragen + Registry (<300ms)
    $cs   = Get-CimInstance Win32_ComputerSystem  -ErrorAction SilentlyContinue
    $bios = Get-CimInstance Win32_BIOS            -ErrorAction SilentlyContinue
    $cpu  = Get-CimInstance Win32_Processor       -ErrorAction SilentlyContinue | Select-Object -First 1
    $os   = Get-CimInstance Win32_OperatingSystem -ErrorAction SilentlyContinue
    $disks  = @(Get-PhysicalDisk                  -ErrorAction SilentlyContinue)
    $parts  = @(Get-Partition                     -ErrorAction SilentlyContinue | Sort-Object DiskNumber, PartitionNumber)
    $volumes= @(Get-Volume                        -ErrorAction SilentlyContinue)
    $missing= @(Get-CimInstance Win32_PnPEntity   -ErrorAction SilentlyContinue | Where-Object { $_.ConfigManagerErrorCode -ne 0 })
    $defender = Get-MpComputerStatus              -ErrorAction SilentlyContinue
    $activation = (& cscript.exe //Nologo "$env:windir\system32\slmgr.vbs" /xpr 2>$null | Out-String).Trim()

    # CHANGED: Win11-Erkennung via Registry Build-Nummer (>= 22000 = Win11)
    $osReg   = Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion" -ErrorAction SilentlyContinue
    $build   = if ($osReg.CurrentBuildNumber) { [int]$osReg.CurrentBuildNumber } else { [int]$os.BuildNumber }
    $dispVer = $osReg.DisplayVersion   # z.B. "24H2"
    $ubr     = $osReg.UBR             # Update Build Revision
    $winName = if ($build -ge 22000) { "Windows 11" } else { "Windows 10" }
    $winEd   = $os.Caption -replace "Microsoft Windows (10|11)\s*",""
    $winFull = "$winName $($winEd.Trim()) $dispVer (Build $build)"
    $winVer  = "$($os.Version)" + $(if ($ubr) { ".$ubr" } else { "" }) + $(if ($dispVer) { " ($dispVer)" } else { "" })
    ...
    [PSCustomObject]@{
        ...
        Windows        = $winFull        # bereits via Registry + Build berechnet
        WindowsVersion = $winVer         # bereits via Registry + UBR berechnet
        ...
        QuickSupport   = if (Test-Path $Script:QuickSupportExe) { $Script:QuickSupportExe } else { "Nicht vorhanden" }
        RunLog         = $Script:RunLogFile
    }
}

function New-LedvHandoutText {
    param(
        [object]$Data,
        [string]$CustomerName,
        [string]$CustomerUser,
    ...
    $nl = [Environment]::NewLine
    $software = Get-LedvInstalledSoftwareSummary
    $actions = Get-LedvActionSummary
    $out = "L-EDV Notebook Handout / Uebergabeprotokoll$nl"
    $out += "================================================$nl$nl"
    $out += "Kunde       : $CustomerName$nl"
    $out += "Benutzer    : $CustomerUser$nl"
    if ($IncludePassword) { $out += "Passwort    : $CustomerPassword$nl" }
    else { $out += "Passwort    : nicht dokumentiert$nl" }
    $out += "Techniker   : $Technician$nl"
    $out += "Datum       : $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')$nl$nl"
    $out += "Geraet$nl-----$nl"
    $out += "Computer    : $($Data.ComputerName)$nl"
    $out += "Hersteller  : $($Data.Manufacturer)$nl"
    $out += "Modell      : $($Data.Model)$nl"
    $out += "Seriennr.   : $($Data.SerialNumber)$nl"
    $out += "CPU         : $($Data.CPU)$nl"
    $out += "RAM         : $($Data.RAMGB) GB$nl$nl"
    $out += "Windows$nl-------$nl"
    $out += "Edition     : $($Data.Windows)$nl"
    $out += "Version     : $($Data.WindowsVersion)$nl"
    $out += "Installiert : $($Data.InstallDate)$nl"
    $out += "Letzter Boot: $($Data.LastBoot)$nl"
    $out += "Aktivierung : $($Data.Activation)$nl$nl"
    $out += "Datentraeger$nl------------$nl"
    foreach ($d in $Data.Disks) { $out += "- $($d.FriendlyName) | $([math]::Round($d.Size/1GB,0)) GB | $($d.MediaType) | $($d.HealthStatus)$nl" }
    $out += "$nlPartitionen$nl-----------$nl"
    foreach ($p in $Data.Partitions) {
        $dl = if ($p.DriveLetter) { "$($p.DriveLetter):" } else { "-" }
        $vol = $Data.Volumes | Where-Object { $_.DriveLetter -eq $p.DriveLetter } | Select-Object -First 1
        $fs = if ($vol) { $vol.FileSystem } else { "" }
        $free = if ($vol -and $vol.SizeRemaining) { " | frei $([math]::Round($vol.SizeRemaining/1GB,1)) GB" } else { "" }
        $out += "- Disk $($p.DiskNumber), Part $($p.PartitionNumber), $dl | $([math]::Round($p.Size/1GB,1)) GB | $fs$free$nl"
    }
    $out += "$nlInstallierte / bereitgestellte Software$nl------------------------------------$nl"
    foreach ($s in $software) { $out += "- $s$nl" }
    $out += "$nlDurchgefuehrte Aenderungen$nl-------------------------$nl"
    foreach ($a in $actions) { $out += "- $a$nl" }
    $out += "$nlSicherheit und Wartung$nl--------------------$nl"
    if ($Data.Defender) {
        $out += "- Defender Echtzeitschutz: $($Data.Defender.RealTimeProtectionEnabled)$nl"
        $out += "- Defender Signaturen   : $($Data.Defender.AntivirusSignatureLastUpdated)$nl"
    } else { $out += "- Defender Status nicht abrufbar$nl" }
    $out += "- TeamViewerQS: $($Data.QuickSupport)$nl"
    $out += "$nlGeraete mit Fehlern$nl-------------------$nl"
    if ($Data.MissingDevices.Count -gt 0) { foreach ($m in $Data.MissingDevices) { $out += "- [$($m.ConfigManagerErrorCode)] $($m.Name)$nl" } }
    else { $out += "- Keine fehlerhaften/fehlenden Geraete gefunden$nl" }
    $out += "$nlBemerkungen$nl-----------$nl$Notes$nl"
    $out += "${nl}Hinweis: Passwortangaben werden nur in diesem Handout ausgegeben, wenn dies explizit ausgewaehlt wurde. Interne Toolkit-Logs enthalten kein eingegebenes Passwort.$nl"
    return $out
}

function New-LedvHandoutHtml {
    param(
        [object]$Data,
        [string]$CustomerName,
        [string]$CustomerUser,
    ...
    $software = Get-LedvInstalledSoftwareSummary
    $actions = Get-LedvActionSummary
    $passwordText = if ($IncludePassword) { ConvertTo-LedvHtmlText $CustomerPassword } else { "nicht dokumentiert" }
    $html = @"
<!doctype html>
<html>
<head>
<meta charset="utf-8">
<title>L-EDV Notebook Handout</title>
<style>
body{font-family:Segoe UI,Arial,sans-serif;margin:32px;color:#1d1d1d;background:#fff}
h1{color:#7d1e2e;margin-bottom:4px}
h2{border-bottom:2px solid #7d1e2e;padding-bottom:4px;margin-top:28px;color:#5c1520}
.meta{color:#555;margin-bottom:24px}
table{border-collapse:collapse;width:100%;margin:8px 0 18px 0}
td,th{border:1px solid #ddd;padding:7px;text-align:left;vertical-align:top}
th{background:#eee4e3}
ul{margin-top:6px}
.warn{border:1px solid #7d1e2e;background:#fff4f4;padding:10px;margin-top:24px}
</style>
</head>
<body>
<h1>L-EDV Notebook Handout</h1>
<div class="meta">Erstellt am $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')</div>
<h2>Kunde</h2>
<table>
<tr><th>Kunde</th><td>$(ConvertTo-LedvHtmlText $CustomerName)</td></tr>
<tr><th>Benutzer</th><td>$(ConvertTo-LedvHtmlText $CustomerUser)</td></tr>
<tr><th>Passwort</th><td>$passwordText</td></tr>
<tr><th>Techniker</th><td>$(ConvertTo-LedvHtmlText $Technician)</td></tr>
</table>
<h2>Geraet</h2>
<table>
<tr><th>Computername</th><td>$(ConvertTo-LedvHtmlText $Data.ComputerName)</td></tr>
<tr><th>Hersteller</th><td>$(ConvertTo-LedvHtmlText $Data.Manufacturer)</td></tr>
<tr><th>Modell</th><td>$(ConvertTo-LedvHtmlText $Data.Model)</td></tr>
<tr><th>Seriennummer</th><td>$(ConvertTo-LedvHtmlText $Data.SerialNumber)</td></tr>
<tr><th>CPU</th><td>$(ConvertTo-LedvHtmlText $Data.CPU)</td></tr>
<tr><th>RAM</th><td>$($Data.RAMGB) GB</td></tr>
</table>
<h2>Windows</h2>
<table>
<tr><th>Edition / Build</th><td>$(ConvertTo-LedvHtmlText $Data.Windows)</td></tr>
<tr><th>Version</th><td>$(ConvertTo-LedvHtmlText $Data.WindowsVersion)</td></tr>
<tr><th>Installiert</th><td>$(ConvertTo-LedvHtmlText $Data.InstallDate)</td></tr>
<tr><th>Letzter Start</th><td>$(ConvertTo-LedvHtmlText $Data.LastBoot)</td></tr>
<tr><th>Aktivierung</th><td><pre>$(ConvertTo-LedvHtmlText $Data.Activation)</pre></td></tr>
</table>
<h2>Datentraeger</h2>
<ul>
"@
    foreach ($d in $Data.Disks) { $html += "<li>$(ConvertTo-LedvHtmlText "$($d.FriendlyName) | $([math]::Round($d.Size/1GB,0)) GB | $($d.MediaType) | $($d.HealthStatus)")</li>`r`n" }
    $html += "</ul><h2>Partitionen</h2><table><tr><th>Disk</th><th>Partition</th><th>Laufwerk</th><th>Groesse</th><th>Dateisystem</th><th>Frei</th></tr>"
    foreach ($p in $Data.Partitions) {
        $vol = $Data.Volumes | Where-Object { $_.DriveLetter -eq $p.DriveLetter } | Select-Object -First 1
        $drive = if ($p.DriveLetter) { "$($p.DriveLetter):" } else { "-" }
        $free = if ($vol -and $vol.SizeRemaining) { "$([math]::Round($vol.SizeRemaining/1GB,1)) GB" } else { "" }
        $html += "<tr><td>$($p.DiskNumber)</td><td>$($p.PartitionNumber)</td><td>$drive</td><td>$([math]::Round($p.Size/1GB,1)) GB</td><td>$(ConvertTo-LedvHtmlText $vol.FileSystem)</td><td>$free</td></tr>"
    }
    $html += "</table><h2>Installierte / bereitgestellte Software</h2><ul>"
    foreach ($s in $software) { $html += "<li>$(ConvertTo-LedvHtmlText $s)</li>`r`n" }
    $html += "</ul><h2>Durchgefuehrte Aenderungen</h2><ul>"
    foreach ($a in $actions) { $html += "<li>$(ConvertTo-LedvHtmlText $a)</li>`r`n" }
    $html += "</ul><h2>Sicherheit, Fernwartung und Geraete</h2><table>"
    if ($Data.Defender) {
        $html += "<tr><th>Defender Echtzeitschutz</th><td>$($Data.Defender.RealTimeProtectionEnabled)</td></tr>"
        $html += "<tr><th>Defender Signaturen</th><td>$(ConvertTo-LedvHtmlText $Data.Defender.AntivirusSignatureLastUpdated)</td></tr>"
    }
    $html += "<tr><th>TeamViewerQS</th><td>$(ConvertTo-LedvHtmlText $Data.QuickSupport)</td></tr>"
    $html += "<tr><th>Toolkit Log</th><td>$(ConvertTo-LedvHtmlText $Data.RunLog)</td></tr></table>"
    $html += "<h2>Geraete mit Fehlern</h2><ul>"
    if ($Data.MissingDevices.Count -gt 0) { foreach ($m in $Data.MissingDevices) { $html += "<li>[$($m.ConfigManagerErrorCode)] $(ConvertTo-LedvHtmlText $m.Name)</li>" } }
    else { $html += "<li>Keine fehlerhaften/fehlenden Geraete gefunden</li>" }
    $html += "</ul><h2>Bemerkungen</h2><p>$(ConvertTo-LedvHtmlText $Notes)</p>"
    $html += "<div class='warn'>Passwoerter werden nur in diesem Handout ausgegeben, wenn dies ausdruecklich ausgewaehlt wurde. Interne Toolkit-Logs enthalten kein eingegebenes Passwort.</div>"
    $html += "</body></html>"
    return $html
}

function Show-LedvHandoutForm {
    $data = Get-LedvSystemHandoutData
    $f = New-Object System.Windows.Forms.Form
    $f.Text = "L-EDV Handout erstellen"
    $f.Size = New-Object System.Drawing.Size(860, 760)
    $f.StartPosition = "CenterParent"
    $f.BackColor = $cBG
    $f.ForeColor = $cText
    $f.Font = $fMain

    Make-Hdr $f "Kundendaten und Uebergabe" 12 12 816
    Make-Lbl $f "Kunde:" 16 52 120 18 $fSmall $cSub | Out-Null
    $txCustomer = New-Object System.Windows.Forms.TextBox
    $txCustomer.Location = New-Object System.Drawing.Point(140,48)
    $txCustomer.Size = New-Object System.Drawing.Size(250,24)
    $txCustomer.BackColor = $cPanel2; $txCustomer.ForeColor = $cText
    $f.Controls.Add($txCustomer)

    Make-Lbl $f "Benutzer:" 410 52 120 18 $fSmall $cSub | Out-Null
    $txUser = New-Object System.Windows.Forms.TextBox
    $txUser.Location = New-Object System.Drawing.Point(530,48)
    $txUser.Size = New-Object System.Drawing.Size(250,24)
    $txUser.BackColor = $cPanel2; $txUser.ForeColor = $cText
    $f.Controls.Add($txUser)

    Make-Lbl $f "Passwort:" 16 84 120 18 $fSmall $cSub | Out-Null
    $txPassword = New-Object System.Windows.Forms.TextBox
    $txPassword.Location = New-Object System.Drawing.Point(140,80)
    $txPassword.Size = New-Object System.Drawing.Size(250,24)
    $txPassword.BackColor = $cPanel2; $txPassword.ForeColor = $cText
    $txPassword.UseSystemPasswordChar = $true
    $f.Controls.Add($txPassword)

    $ckPassword = New-Object System.Windows.Forms.CheckBox
    $ckPassword.Text = "Passwort im Handout ausgeben"
    $ckPassword.Location = New-Object System.Drawing.Point(410,82)
    $ckPassword.Size = New-Object System.Drawing.Size(250,22)
    $ckPassword.ForeColor = $cWarn
    $ckPassword.FlatStyle = "Flat"
    $f.Controls.Add($ckPassword)

    Make-Lbl $f "Techniker:" 16 116 120 18 $fSmall $cSub | Out-Null
    $txTech = New-Object System.Windows.Forms.TextBox
    $txTech.Location = New-Object System.Drawing.Point(140,112)
    $txTech.Size = New-Object System.Drawing.Size(250,24)
    $txTech.BackColor = $cPanel2; $txTech.ForeColor = $cText
    $txTech.Text = $env:USERNAME
    $f.Controls.Add($txTech)

    $ckConfirm = New-Object System.Windows.Forms.CheckBox
    $ckConfirm.Text = "Kunde hat Einrichtung / dokumentierte Aenderungen bestaetigt"
    $ckConfirm.Location = New-Object System.Drawing.Point(410,114)
    $ckConfirm.Size = New-Object System.Drawing.Size(390,22)
    $ckConfirm.ForeColor = $cText
    $ckConfirm.FlatStyle = "Flat"
    $f.Controls.Add($ckConfirm)

    Make-Lbl $f "Bemerkungen:" 16 150 120 18 $fSmall $cSub | Out-Null
    $txNotes = New-Object System.Windows.Forms.TextBox
    $txNotes.Location = New-Object System.Drawing.Point(140,146)
    $txNotes.Size = New-Object System.Drawing.Size(640,58)
    $txNotes.Multiline = $true
    $txNotes.ScrollBars = "Vertical"
    $txNotes.BackColor = $cPanel2; $txNotes.ForeColor = $cText
    $f.Controls.Add($txNotes)

    Make-Hdr $f "Automatisch ermittelte Vorschau" 12 218 816
    $preview = New-Object System.Windows.Forms.TextBox
    $preview.Location = New-Object System.Drawing.Point(16,250)
    $preview.Size = New-Object System.Drawing.Size(812,360)
    ...
    $refreshPreview = {
        $preview.Text = New-LedvHandoutText -Data $data -CustomerName $txCustomer.Text -CustomerUser $txUser.Text -CustomerPassword $txPassword.Text -IncludePassword $ckPassword.Checked -Technician $txTech.Text -Notes $txNotes.Text
    }
    & $refreshPreview

    Make-Btn $f "Vorschau aktualisieren" 16 628 180 34 $cPanel2 $refreshPreview | Out-Null
    Make-Btn $f "Handout speichern" 206 628 180 34 $cAccent {
        if (-not $ckConfirm.Checked) {
            $r = [System.Windows.Forms.MessageBox]::Show("Die Kundenbestaetigung ist nicht angehakt. Trotzdem speichern?","Hinweis","YesNo","Warning")
            if ($r -ne "Yes") { return }
        }
        $safeCustomer = ($txCustomer.Text -replace '[^\w\- ]','').Trim()
        if (-not $safeCustomer) { $safeCustomer = "Kunde" }
        $stamp = Get-Date -Format "yyyy-MM-dd_HH-mm-ss"
        $txtFile = Join-Path $Script:LedvFilesDir ("Handout-$safeCustomer-$stamp.txt")
        $htmlFile = Join-Path $Script:LedvFilesDir ("Handout-$safeCustomer-$stamp.html")
        $txt = New-LedvHandoutText -Data $data -CustomerName $txCustomer.Text -CustomerUser $txUser.Text -CustomerPassword $txPassword.Text -IncludePassword $ckPassword.Checked -Technician $txTech.Text -Notes $txNotes.Text
        $html = New-LedvHandoutHtml -Data $data -CustomerName $txCustomer.Text -CustomerUser $txUser.Text -CustomerPassword $txPassword.Text -IncludePassword $ckPassword.Checked -Technician $txTech.Text -Notes $txNotes.Text
        $txt | Out-File $txtFile -Encoding UTF8
        [System.IO.File]::WriteAllText($htmlFile, $html, [System.Text.Encoding]::UTF8)
        Write-Log "Handout gespeichert: $htmlFile / $txtFile"
        $txPassword.Text = ""
        $preview.Text = $txt
        [System.Windows.Forms.MessageBox]::Show("Handout gespeichert:`n$htmlFile`n$txtFile","L-EDV","OK","Information") | Out-Null
    } | Out-Null
    Make-Btn $f "Handout oeffnen" 396 628 160 34 $cPanel2 {
        Start-Process "explorer.exe" $Script:LedvFilesDir
    } | Out-Null
    Make-Btn $f "Schliessen" 668 628 160 34 $cPanel2 { $f.Close() } | Out-Null

    [void]$f.ShowDialog($Form)
}

Make-Btn $pgA "Handout Formular" 728 46 170 30 $cAccent2 {
    Show-LedvHandoutForm
    Write-Log "Handout Formular geoeffnet"
} | Out-Null
...
$Form.Add_FormClosed({
    ...
    # Timer und Runspace sauber beenden
    if ($Script:LedvTimer)    { $Script:LedvTimer.Stop(); $Script:LedvTimer.Dispose() }
    if ($Script:LedvRunspace) { $Script:LedvRunspace.Close(); $Script:LedvRunspace.Dispose() }
