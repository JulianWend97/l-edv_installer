...
$Form.Text            = "L-EDV Toolkit  |  IT-Support v1.9"
...
$lblHeaderSub.Text      = "Windows L-EDV Toolkit  |  IT-Support v1.9"
...
function Get-LedvSystemHandoutData {
    ...
    [PSCustomObject]@{
        ...
        Windows = if ($ci) { "$($ci.WindowsProductName) Build $($ci.OsBuildNumber)" } else { $os.Caption }
        WindowsVersion = if ($ci) { $ci.WindowsVersion } else { $os.Version }
