...
foreach ($d in @($Script:LogDir, $Script:PortableDir)) {
...
$tabNames = @("System","Legacy Panels","Software","Tools","Portable Apps","Wartung","Debloat","LEDV Installer")
...
$Script:btnInstallTvQs = Make-Btn $spSW "TeamViewer QuickSupport installieren" 278 60 260 34 $cPanel2 {
    ...
    try {
        ...
        [System.Windows.Forms.MessageBox]::Show("TeamViewer QuickSupport heruntergeladen.`nDesktop-Verknuepfung erstellt.","L-EDV","OK","Information") | Out-Null
    } catch {
...
$Script:btnStartTvQs = Make-Btn $spSW "TV QuickSupport starten" 548 60 200 34 $cPanel2 {
    $exe = "C:\Software\TeamViewerQS_x64.exe"
    if (Test-Path $exe) {
        Start-Process $exe
...
Make-Btn $spSW "Host weg + QS" 758 60 150 34 $cOK {
    ...
    $qsExe = "C:\Software\TeamViewerQS_x64.exe"
    if (Test-Path $qsExe) {
...
Make-Lbl $pgP ("Ordner: " + $Script:PortableDir + "  (alle .exe werden automatisch als Button angezeigt)") 8 38 880 14
...
function Load-PortableApps {
    ...
    if (-not (Test-Path $Script:PortableDir)) { New-Item -ItemType Directory $Script:PortableDir -Force | Out-Null }
    $exes = Get-ChildItem $Script:PortableDir -Filter "*.exe" -ErrorAction SilentlyContinue | Sort-Object Name

    if ($exes.Count -eq 0) {
        $lbl = New-Object System.Windows.Forms.Label
        $lbl.Text      = "Keine .exe-Dateien in portable_apps gefunden." + [Environment]::NewLine + "EXE-Dateien in den Ordner legen und dann 'Neu laden' klicken."
        $lbl.Font      = $fMono
        $lbl.ForeColor = $cSub
    ...
    foreach ($exe in $exes) {
        ...
        $y   = $row * 78
        $ep  = $exe.FullName
        $an  = [System.IO.Path]::GetFileNameWithoutExtension($exe.Name)
...
foreach ($d in $bloatDefs) {
    $cb = New-ADTCheck $Script:DebloatPages["Bloatware"] $d[0] (14+($col*296)) (18+($row*22)) 284 ([bool]$d[2])
...
foreach($d in $privDefs){ $Script:dbPriv[$d[0]] = New-ADTCheck $Script:DebloatPages["Datenschutz"] $d[1] $d[3] $d[4] 410 ([bool]$d[2]) }
...
$col=0;$row=0
foreach($d in $xboxDefs){ $fg=if($d[3]){$cWarn}else{$cText}; $Script:dbXbox[$d[0]]=New-ADTCheck $Script:DebloatPages["Xbox Gaming"] $d[1] (14+($col*430)) (40+($row*24)) 410 ([bool]$d[2]) $fg; $col++; if($col -ge 2){$col=0;$row++} }
...
$Script:dbExplorer["ClassicRC"] = New-ADTCheck $pgRC "Klassisches Rechtsklick-Menue (Win10-Stil)" 14 45 500 $true
$Script:dbExplorer["EndTask"] = New-ADTCheck $pgRC "Task beenden im Rechtsklick auf Taskleiste aktivieren" 14 69 500 $true
$explDefs = @(
...
foreach($d in $explDefs){ $Script:dbExplorer[$d[0]] = New-ADTCheck $pgRC $d[1] $d[2] $d[3] 410 ([bool]$d[4]) }
...
$swDefs = @(
    ...
    @("Greenshot","Greenshot","greenshot",228,146,$true),@("Teams","Microsoft Teams","microsoft-teams",228,170,$false)
)
foreach($d in $swDefs){ $Script:ledvChecks[$d[0]] = New-ADTCheck $spI $d[1] $d[3] $d[4] 200 ([bool]$d[5]) }
...
$y=74
foreach($d in $twDefs){ $fg=if($d[0] -eq "OneDrive"){$cWarn}else{$cText}; $Script:ledvChecks[$d[0]] = New-ADTCheck $spI $d[1] 474 $y 410 ([bool]$d[2]) $fg; $y += 24 }

Make-Hdr $spI "Ausfuehren" 8 390 888
Make-Lbl $spI "Kundenname:" 16 426 100 16 $fSmall $cSub | Out-Null
$Script:txLedvCustomer = New-Object System.Windows.Forms.TextBox
$Script:txLedvCustomer.Location = New-Object System.Drawing.Point(116,422)
...
Make-Btn $spI "Ausgewaehlte Installation starten" 370 418 260 36 $cAccent { Invoke-AdminLedvInstall } | Out-Null

function Invoke-AdminDownloadTVQS {
    $dir="C:\Software"; if(-not(Test-Path $dir)){New-Item -ItemType Directory -Path $dir -Force|Out-Null}
    $exe="$dir\TeamViewerQS_x64.exe"; $url="https://customdesignservice.teamviewer.com/download/windows/v15/6mes7x8/TeamViewerQS_x64.exe"
    [System.Net.ServicePointManager]::SecurityProtocol=[System.Net.SecurityProtocolType]::Tls12
...
function Invoke-AdminRemoveTeamViewerHost {
    Write-Log "LEDV: TeamViewer Host entfernen"
    Get-Process "TeamViewer*" -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
    foreach($sn in @("TeamViewer","TeamViewer_Service","TeamViewer_Desktop")){ if(Get-Service $sn -ErrorAction SilentlyContinue){Stop-Service $sn -Force -ErrorAction SilentlyContinue; Set-Service $sn -StartupType Disabled -ErrorAction SilentlyContinue} }
    foreach($root in @("HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall","HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall")){
        Get-ChildItem $root -ErrorAction SilentlyContinue | ForEach-Object { $p=Get-ItemProperty $_.PSPath -ErrorAction SilentlyContinue; if($p.DisplayName -like "*TeamViewer*" -and $p.DisplayName -notlike "*QuickSupport*" -and $p.UninstallString){ if($p.UninstallString -match "\{[^}]+\}"){$guid=[regex]::Match($p.UninstallString,"\{[^}]+\}").Value; Start-Process msiexec.exe "/x $guid /qn /norestart" -Wait -WindowStyle Hidden -ErrorAction SilentlyContinue}else{Start-Process cmd.exe "/c `"$($p.UninstallString)`" /S" -Wait -WindowStyle Hidden -ErrorAction SilentlyContinue}; Write-Log "LEDV: deinstalliert $($p.DisplayName)" } }
    }
    foreach($p in @("$env:ProgramFiles\TeamViewer","${env:ProgramFiles(x86)}\TeamViewer","$env:ProgramData\TeamViewer")){ if(Test-Path $p){Remove-Item $p -Recurse -Force -ErrorAction SilentlyContinue} }
    foreach($p in @("HKLM:\SOFTWARE\TeamViewer","HKLM:\SOFTWARE\WOW6432Node\TeamViewer")){ if(Test-Path $p){Remove-Item $p -Recurse -Force -ErrorAction SilentlyContinue} }
}
...
function Invoke-AdminLedvInstall {
    ...
    if(-not(Get-Command choco -ErrorAction SilentlyContinue)){Set-Status "Chocolatey wird eingerichtet..." 8; Set-ExecutionPolicy Bypass -Scope Process -Force; [System.Net.ServicePointManager]::SecurityProtocol=[System.Net.SecurityProtocolType]::Tls12; Invoke-Expression ((New-Object System.Net.WebClient).DownloadString('https://chocolatey.org/install.ps1')); $env:PATH="$env:ChocolateyInstall\bin;$env:PATH"; Write-Log "LEDV: Chocolatey eingerichtet"}
    ...
    if($Script:ledvChecks["DesktopIcons"].Checked){Set-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" HideIcons -Type DWord -Value 0 -ErrorAction SilentlyContinue}
